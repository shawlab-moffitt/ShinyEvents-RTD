fetch_csv_from_api <- function(url,auth) {
  ...
  txt <- content(resp2, as = "text", encoding = "UTF-8")
  # Parse CSV text into a data frame
  t = read.csv(text = txt, stringsAsFactors = FALSE,check.names = F)
}

prep_time_axis <- function(df, start_col, stop_col, unit = "years", date_format = "%m/%d/%Y") {
  unit <- tolower(unit)
  if (unit %in% c("date", "dates")) {
    df[[start_col]] <- as.Date(df[[start_col]], format = date_format)
    df[[stop_col]]  <- as.Date(df[[stop_col]],  format = date_format)
  } else {
    df[[start_col]] <- as.numeric(df[[start_col]])
    df[[stop_col]]  <- as.numeric(df[[stop_col]])
  }
  df
}
...
timelinePlot <- function(data = NULL,event_col = NULL, event_type_col = NULL, start_col = NULL, stop_col = NULL, unit = "years", date_format = "%m/%d/%Y",
  ...
  if (tolower(unit) %in% c("date", "dates")) {
    if (!inherits(data[[start_col]], "Date")) stop("Event start time could not be converted to Date")
    if (!inherits(data[[stop_col]], "Date")) stop("Event stop time could not be converted to Date")
  } else {
    if (!is.numeric(data[[start_col]])) stop("Event start time must be numeric")
    if (!is.numeric(data[[stop_col]])) stop("Event stop time must be numeric")
  }
  ...
  if (!is.null(highlight_col)) {
    if (highlight_col %in% colnames(data)) {
      ...
      ggplot2::geom_vline(
        xintercept = if (tolower(unit) %in% c("date", "dates")) {
          as.numeric(highligh_data[[start_col]])
        } else {
          highligh_data[[start_col]]
        },
  ...
  if (tolower(unit) %in% c("date", "dates")) {
    plot2 <- plot2 +
      ggplot2::scale_x_date(
        limits = range(allAges, na.rm = TRUE),
        date_breaks = "1 month",
        date_labels = "%b %Y"
      )
  } else {
